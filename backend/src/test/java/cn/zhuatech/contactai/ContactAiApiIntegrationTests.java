 ...
 */
@SpringBootTest @AutoConfigureMockMvc class ContactAiApiIntegrationTests { @Autowired MockMvc mvc; private String token;
 /**
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
  ...
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
 @Test void operatorCanAnalyzeConversation()throws Exception{mvc.perform(post("/api/ai/contact/analyze").header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"conversationId\":\"C-88\",\"sentimentScore\":30,\"waitSeconds\":160,\"repeatContacts\":2,\"compliancePhraseMissed\":true,\"refundAmount\":6800,\"vip\":false}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.severity").value("CRITICAL"));}}
